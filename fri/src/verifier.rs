        ...
        >,
{
    ...
    // Validate that the height derived from the reduction schedule matches the expected height.
    ...
    // Ensure the height is safe for the field.
    if log_global_max_height >= Val::bits() {
        return Err(FriError::InvalidProofShape);
    }
