        ...
        >,
{
    ...
    // Validate log_global_max_height against field and system limits.
    if log_global_max_height > Val::TWO_ADICITY || log_global_max_height >= usize::BITS as usize {
        return Err(FriError::InvalidProofShape);
    }

    // Ensure that the derived log_global_max_height matches the expected height from the input matrices.
