        ...
        >,
{
    ...
    // Validate that log_global_max_height is within bounds.
    if log_global_max_height > Val::TWO_ADICITY
        || log_global_max_height >= (usize::BITS as usize).saturating_sub(1)
    {
        return Err(FriError::InvalidProofShape);
    }
    ...
    if log_global_max_height != expected_max_height {
        return Err(FriError::InvalidProofShape);
    }
